...
class TickTock(SingleServerIRCBot):
    ...
    def game_Controls(self, c, e, msg, sender):
        pass
        # Check to see if a game is active
        # Determine whose turn it is
        # Draw Cards
        # Accept Item, Recipe, Arte, Info and Resolve commands, resolving the item and recipe commands
        # Resolve the turn
        # Switch state to next player's turn
    ...
    def sekrit_Commands(self, c, e, msg, sender):
        ...
        if msg[:5] == 'Deck ':
            try:
                deckNum = int(msg[5:])
                self.decks[self.players.index(sender)] = Deck(sender, deckNum)
